...
- Pourquoi c’est un fit:
  - Cœur du rôle = **acceptance/validation** sur systèmes déployés, avec outillage (scripts, métriques, analyse d’écarts) → mapping direct sur ton axe `validation/calibration/diagnostic`.
...
- Liens (vérifiés le `2026-04-27`):
  - Offre + candidature (Recruitee, formulaire “Envoyer” visible sur la page): https://kontrontransportation.recruitee.com/o/ingenieur-integrationvalidation-reseaux-telecoms?lang=fr
- Pourquoi c’est un fit:
...
- Pourquoi c’est un fit “technique”:
  - Simulation + campagnes de simulation + validation d’algos → cœur “simulation & validation”.
...
### C2. Novaspace — Senior Consultant - Satcom Constellation Design and Simulation (Toulouse, On-site)

- Liens (vérifiés le `2026-04-27`):
  - Offre (Lever): https://jobs.lever.co/novaspace/b21dc3c9-0d19-4c26-8319-f23913c014cc
  - Candidature (formulaire “Submit application”): https://jobs.lever.co/novaspace/b21dc3c9-0d19-4c26-8319-f23913c014cc/apply
- Pourquoi ça peut valoir un dossier (malgré stretch):
  - Thème “design & simulation de constellation” → rôle orienté simulation/systèmes, avec analyses quantitatives.
  - Position conseil: produire des analyses défendables, cadrer des hypothèses et des risques → compatible avec ton axe “validation/robustesse/diagnostic”.
- Risque/gap principal:
  - Pré-requis affichés très “satcom” + niveau “Senior” (expérience constellation + ~7 ans) → stretch domaine + seniorité; candidater seulement si tu acceptes un pari.

#### CV title (tailored)

`Scientific software / simulation engineer (Python/C++) — validation, robustesse, analyses défendables`

#### Summary (tailored)

Profil R&D orienté simulation et crédibilité: je construis des analyses quantitatives reproductibles en `Python/C++` et je valide des modèles/pipelines sous incertitude (bruit, biais, mismatch).
Je peux apporter une rigueur de validation (stress tests, diagnostics, reporting) — tout en restant factuelle sur un gap satcom/constellation.

#### Top 6 bullets à mettre en tête (à réécrire/mettre en avant)

1. Built reusable `Python/C++` pipelines for modelling and validation with traceable outputs and documented checks.
2. Implemented probabilistic modelling and inference under imperfect observations; quantified uncertainty and limitations.
3. Ran sensitivity studies by varying assumptions; compared scenarios and communicated trade-offs clearly.
4. Designed calibration/correction workflows to reduce structured bias; quantified before/after effects.
5. Built monitoring/validation routines that surfaced anomalies and supported root-cause diagnosis.
6. Produced decision-ready technical reporting (assumptions, risks, metrics) for stakeholders.
...
- Outsight — Senior System Integration Engineer: page offre accessible mais impossible de confirmer le formulaire de candidature (erreur “cache miss” côté outil lors de l’ouverture du chemin officiel “apply” le `2026-04-27`) → **exclu** (“apply” non vérifiable).
  - Offre repérée: https://jobs.lever.co/outsight/26f37bc5-b59b-42e8-80a1-117d59bec989

### Exclus — hors-cible / contraintes fortes (exemples)

- Kontron Transportation — Ingénieur Intégration et Acceptance Réseaux d’Accès Radio: très orienté RAN + déplacements annoncés, plus éloigné d’une trajectoire “simulation/validation systèmes physiques” → dépriorisé dans cette track (offre ouverte et candidature possible).
  - Offre + candidature (Recruitee): https://kontrontransportation.recruitee.com/o/ingenieur-integration-et-acceptance-reseaux-dacces-radio?lang=fr

### Exclus — repérés via LinkedIn mais non vérifiables (login wall)

- Exemples: offres LinkedIn “Simulation/Validation Engineer” repérées mais impossibles à vérifier côté chemin officiel de candidature sans connexion LinkedIn → **exclues** (“apply” non vérifiable).
